...
- **`_捨て/` の見回り**: `review` で隔離ファイルを一覧（隔離日時・元の場所・理由つき）、
  ...
  必ず `--yes` ゲート）
- **操作履歴の横断タイムライン**: apply / undo / redo / review の成功時、その1行が
  `~/.tidy/history.jsonl` に集約される。`history` でどの場所を整理したかを1本の時系列で辿れる
  （`--target` で絞り込み）。実ファイルは動かさず来歴の索引だけをホームに置く設計
- **テスト済み**: `tests/run_tests.py` に round-trip・重複排除・衝突・安全性・中断耐性・診断・
  suggest・verify・redo・自己移動拒否・サイズ表示・**コード依存解析**（py/js/html/css）・
  **クラスタ分断警告**・**cross-target 単一ソース**・**review (list/restore/purge)**・
  **history (記録/タイムライン/フィルタ/home 隔離)** など **114 項目** の総合テスト
...
# 8. 操作履歴（v6・全ての場所を横断）
python3 scripts/organize.py history                        # 全操作を新しい順に
python3 scripts/organize.py history --target "<宛先ルート>" # 特定ディレクトリだけ
python3 scripts/organize.py history --verbose              # manifest/ログのパスも
```
